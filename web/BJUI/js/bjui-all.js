...
+function ($) {
    ...
    BJUI.StrBuilder = new StrBuilder()
    window.BJUI = BJUI
...
/* ========================================================================
 * B-JUI: bjui-regional.zh-CN.js  v1.3
 * @author K'naan
...
+function ($) {
    ...
    Basedrag.prototype.init = function () {
        ...
        if (this.options.event)
            ...
            if (this.options.selector)
                this.$element.find(this.options.selector).on('mousedown', function (e) {
    ...
    Basedrag.prototype.start = function (e) {
        ...
        $(document).on('mouseup.bjui.basedrag', function (e) {
            ...
            that.$split2.each(function () {
                ...
                $spbar2
                    ...
                    .basedrag($.extend(that.options, {
                        obj: that.$bar, move: 'horizontal', event: e, stop: function () {
...
/* ========================================================================
 * B-JUI: bjui-contextmenu.js  v1.3
 * @author K'naan
    ...
    $(function () {
        var INIT_NAVTAB = function () {
            ...
            $main
                ...
                .click(function () {
                    if (!$(this).hasClass('active')) $(this).navtab('switchTab', 'main')
                })
    ...
    Navtab.prototype.reloadForm = function (clearQuery, option) {
        ...
        if ($tab && $panel) {
            ...
            if ($pagerForm && $pagerForm.length) {
                options.type = $pagerForm.attr('method') || 'POST'

                if ($pagerForm.attr('action'))
                    options.url = $pagerForm.attr('action')
...
+function ($) {
    ...
    Dialog.prototype.reloadForm = function (clearQuery, option) {
        var $dialog, options, datas = $('body').data('bjui.dialog')

        if (typeof option === 'string') {
            $dialog = datas[option].dialog
            options = $dialog.data('options')
        } else if (typeof option === 'object') {
            $dialog = (option.id && datas[option.id].dialog) || this.getCurrent()
        }
        if ($dialog) {
...
+function ($) {
    ...
    Bjuiajax.prototype.ajaxform = function (options) {
        var that = this,
            $target = null,
            $form = (options && options.form) || this.$element,
        ...
        if (!$form || !$form.length || !($form.isTag('form'))) {
            BJUI.debug('Bjuiajax Plugin: \'ajaxform\' method does not set the form!')
            return
        }
        ...
        if (!options || !options.target || !$target || !$target.length) {
            $target = $.CurrentDialog || $.CurrentNavtab
        }
        that.$target = $target

        $.extend(that.options, typeof options === 'object' && options)
        ...
        var _submitFn = function () {
            ...
            // for ie8
            if (BJUI.isIE(8)) {
                if (!$form[0].getAttributeNode('method').specified) op.type = 'post'
            }
    ...
    Bjuiajax.prototype.doload = function (options) {
        var that = this, $target = options.target ? $(options.target) : null
    ...
    Bjuiajax.prototype.refreshlayout = function (target) {
        ...
        if (!$target || !$target.length) {
            if (autorefreshTimer) clearInterval(autorefreshTimer)
            BJUI.debug('No argument, can not refresh DIV.(parameters:[selector/jQuery object/element])')
            return
        }
        if ($target && $target.length) {
            if (!$target.data('options')) {
                BJUI.debug('The target(DIV) is not a reload layout!')
                return
            }
    ...
    Bjuiajax.prototype.reloadlayout = function (options) {
        var $target = options && (options.target ? $(options.target) : null),
        ...
        if (!$target || !$target.length) {
            ...
            if (!$target.data('options')) {
                BJUI.debug('This method is used incorrectly!')
                return
            }
    ...
    Bjuiajax.prototype.doajax = function (options) {
        ...
        if (!options.target || !$target || !$target.length) {
            $target = $.CurrentDialog || $.CurrentNavtab
        }
        that.$target = $target
    ...
    Bjuiajax.prototype.ajaxdownload = function (options) {
        var that = this, $target
        ...
        if (!options.url) {
            BJUI.debug('Error trying to download a file: url is undefined!')
            return
        } else {
            options.url = decodeURI(options.url).replacePlh($target)
            ...
            if (!options.url.isFinishedTm()) {
                BJUI.alertmsg('error', (options.warn || BJUI.regional.plhmsg))
                BJUI.debug('The ajax download url is incorrect: ' + options.url)
                return
            }
        }
    ...
    Bjuiajax.prototype.doajaxchecked = function (options) {
        ...
        var todo = function () {
            ...
            $element.doAjax({
                ...
                url: options.url,
                data: options.data,
                callback: callback ? callback : $.proxy(function (data) {
                    that.ajaxcallback(data)
                }, that)
            })
        }
    ...
    $(document).on('click.bjui.bjuiajax.data-api', '[data-toggle="ajaxdownload"]', function (e) {
        ...
        Plugin.call($this, 'ajaxdownload', options)
                    ...
                    $.each(that.columnModel, function (i, n) {
                        if (n.fixedWidth) {
                            ...
                            if ($firstTr.hasClass("datagrid-nodata")) {
                                ...
                                $th = $tableB.find('> tbody > tr:first-child > td:eq(' + i + ')').css({
                                    width: '',
                                    'white-space': 'nowrap'
                                })
                ...
                switch (op.type) {
                    ...
                    case 'findgrid':
                        ...
                        $td.on('afterchange.bjui.findgrid', '[data-toggle="findgrid"]', function (e, data) {
                            ...
                            if (data.value == changeData[op.name]) {
                                $td.removeClass(that.classnames.td_changed)
                            }

                            changeData[op.name] = data.value
                        })
            ...
            switch (model.type) {
                ...
                case 'findgrid':
                    $form.on('afterchange.bjui.findgrid', '[data-toggle="findgrid"]', function (e, data) {
                        changeData[model.name] = data.value
                    })
...
+function ($) {
    ...
    $(document).on(BJUI.eventType.initUI, function (e) {
        ...
        $box.find('textarea').each(function () {
            var $element = $(this).addClass('form-control'), cols = $element.attr('cols') || 0, width = cols * 10, toggle = $element.attr('toggle')
